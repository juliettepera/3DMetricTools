/* $Id: reporting.cxx,v 1.1 2005/05/24 07:37:58 xushun Exp $ */
...
{
  struct outbuf *ob;
  ob = (struct outbuf *)xa_malloc(sizeof(*ob));
  ob->strbuf = (char *)xa_malloc(OUTBUF_MIN_FREE);
  ob->strbuf[0] = '\0';
  ob->pos = ob->strbuf;
...
void outbuf_printf(struct outbuf *ob, const char *format, ...)
{
  ...
  if (free_sz < OUTBUF_MAX_SZ) {
    ...
    ob->strbuf = (char *)xa_realloc(ob->strbuf,new_sz);
