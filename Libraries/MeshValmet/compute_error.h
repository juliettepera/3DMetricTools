/* $Id: compute_error.h,v 1.3 2011/02/01 15:20:47 xushun Exp $ */
...
struct face_error {
  ...
  double abs_mean_error; /* The absolute mean error for the face -- Christine Xu */
  double mean_sqr_error; /* The mean squared error for the face */
  double abs_mean_sqr_error;/* The absolute mean squared error for the face -- Christine Xu */
...
struct model_error {
  ...
  /* begin -- Christine Xu */
  double abs_min_error;   /* The absolute minimum error value (at sample) */
  double abs_max_error;   /* The absolute maximum error value (at sample) */
  /* end -- Christine Xu */
  ...
  struct face_error *fe;  /* The per-face error metrics. NULL if not
                           ...
                           * be accessed linearly. ---------- !!!!!!!!IMPORTANT!!!!!!! ----------*/
...
struct dist_surf_surf_stats {
  ...
  double abs_max_dist;
  /* end -- Christine Xu */
  double mean_dist; /* Face mean distance from model 1 to model 2 */
  double mean_tot;  /* added by Christine Xu */
  double rms_dist;  /* Root mean squared distance from model 1 to model 2 */
  double rms_tot; /* added by Christine Xu */
  double abs_mean_dist; /* added by Christine Xu */
  double abs_mean_tot; /* added by Christine Xu */  
  double abs_rms_dist; /* added by Christine Xu */
  double abs_rms_tot; /* added by Christine Xu */
 ...
 * to compute the normals, so it is very fast. If prog is not NULL it is used
