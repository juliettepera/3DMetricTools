/* $Id: model_analysis.cxx,v 1.1 2005/05/24 07:37:58 xushun Exp $ */
...
{
  return (unsigned int *)xa_calloc((sz+BMAP_T_BITS-1)/BMAP_T_BITS,sizeof(bmap_t));
}
...
static void stack_grow_buf(struct stack *s)
{
  ...
  s->data = (char *)xa_realloc(s->data,buf_sz);
  s->end = s->data+buf_sz;
...
{
  *sz += 2;
  list->vtcs = (int *)xa_realloc(list->vtcs,*sz*sizeof(*(list->vtcs)));
}
                                ...
                                struct vtx_list *vlist)
{
  ...
  vfaces = (int *)xa_malloc(sizeof(*(vfaces))*n_vfaces);
  memcpy(vfaces,flist->face,sizeof(*(vfaces))*n_vfaces);
  /* do typical size allocation */
  vtx_buf_sz = nf;
  vlist->vtcs = (int *)xa_realloc(vlist->vtcs,sizeof(*(vlist->vtcs))*vtx_buf_sz);
...
static void add_adj_face_extra(struct adj_faces *af, int edge_idx, int face_idx)
{
  ...
  if (af->extra == NULL) { /* no extra lists for no edge yet */
    af->extra = (struct adj_faces_x *)xa_calloc(1,sizeof(*(af->extra)));
  }
  j = (af->extra->n_faces_on_edge[edge_idx])++;
  af->extra->faces_on_edge[edge_idx] =
    (int *)xa_realloc(af->extra->faces_on_edge[edge_idx],
                                              ...
                                              const bmap_t *manifold_vtcs)
{
  ...
  aflist = (struct adj_faces *)xa_malloc(n_faces*sizeof(*aflist));
  for (k=0; k<n_faces; k++) {
 ...
 * index of edge of *face2 that is shared with *face1. Returns zero if both faces have
                               ...
                               struct model_info *minfo)
{
  ...
  stack_init(&stack,sizeof(cur));
  vlist = (struct vtx_list *)xa_calloc(n_vtcs,sizeof(*vlist));
...
struct face_list *faces_of_vertex(const struct model *m, int *n_degenerate)
{
  ...
  fl = (struct face_list *)xa_calloc(m->num_vert,sizeof(*fl));
  ...
  for (j=0, jmax=m->num_vert; j<jmax; j++) {
    fl[j].face = (int *)xa_malloc(fl[j].n_faces*sizeof(*(fl->face)));
    fl[j].n_faces = 0;
  }
