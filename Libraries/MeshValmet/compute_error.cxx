/* $Id: compute_error.cxx,v 1.3 2011/02/01 15:17:31 xushun Exp $ */
...
struct misc_stats {
  double *dist_smpl;/* The distance at each sample of model 1, 
             err and errlin all point to this big array. */
...
//From function realloc_triag_sample_error, 
//err and err_lin are actually pointing to the same content.
...
struct dist_pt_surf_stats {
  ...
  int sum_kmax;           /* the sum of the max k for each sample point */
};
...
static void realloc_triag_sample_error(struct triag_sample_error *tse, int n)
{
  ...
  if (tse->n_samples_tot >  tse->buf_sz) {
    tse->err = (double **)xa_realloc(tse->err,n*sizeof(*(tse->err)));
    tse->err_lin = (double *)xa_realloc(tse->err_lin,
                                           ...
                                           const struct triangle_list *tl)
{
  ...
  m->normals = (vertex_t *)xa_realloc(m->normals,m->num_vert*sizeof(*(m->normals)));
...
/* Christine: Here it uses a random varible to determin the sampling frequency
 * of each triangle, which might be different each time. But the expected frequency
 * won't be too different.
 * For each triagle, n_samples = area*density, density = 1/sqrt(sampling_step*m2bboxdiagonal).
 * Once we get the initial n_samples, we can determine the initial maximum sampling freq. n
 * by the relation that the number of samples is n*(n+1)/2.
 * Then we pick the final sampling freq. depends on the computed initial sampling freq.
 * and the user-determined min sampling freq., whichever is smaller.
 */
                                  ...
                                  int **buf, int *buf_sz)
{
  ...
  dlists->list = (struct cell_list *)xa_realloc(dlists->list,(k+1)*sizeof(*(dlists->list)));
  if (k > dlists->n_dists){
  ...
  if (*buf == NULL || *buf_sz < max_n_cells) {
    *buf_sz = max_n_cells;
    *buf = (int *)xa_realloc(*buf,(*buf_sz)*sizeof(**buf));
  }
  cur_cell = *buf;
  ...
  cll = cur_cell-*buf;
  if (cll != 0) {
    dlists->list[k].cell = (int *)xa_malloc(cll*sizeof(*cur_cell));
...
static struct triangle_list* model_to_triangle_list(const struct model *m)
{
  ...
  n = m->num_faces;
  tl = (struct triangle_list *)xa_malloc(sizeof(*tl));
  tl->n_triangles = n;
  triags = (struct triangle_info *)xa_malloc(sizeof(*tl->triangles)*n);
  tl->triangles = triags;
  tl->area = 0;
                             ...
                             struct misc_stats *m_stats)
{
  ...
  /* begin -- Christine Xu */
  double abs_err_a, abs_err_b, abs_err_c;
  double abs_err_tot, abs_err_sqr_tot;  
  /* end -- Christine Xu */
  double err_min, err_max, err_tot, err_sqr_tot;
  /* begin -- Christine Xu */
  double abs_err_min, abs_err_max;
  /* end -- Christine Xu */
  double **s_err;
  ...
  if (m_stats->dist_smpl_sz < n_tot+dss_stats->m1_samples) {
    ...
    m_stats->dist_smpl = (double *)xa_realloc(m_stats->dist_smpl,
                                    sizeof(*(m_stats->dist_smpl))*
                                    m_stats->dist_smpl_sz);
  }
  // Copy the sample errors in tse->err_lin to m_stats to make a big array;
  // eventually, fe[i]->serror are all pointing to this big array. !!IMPORTANT!!
  memcpy(m_stats->dist_smpl+dss_stats->m1_samples, tse->err_lin,sizeof(*(m_stats->dist_smpl))*n_tot);
  ...
  for (i=0, imax=n-1; i<imax; i++) {
    for (j=0, jmax=imax-i; j<jmax; j++) {
      /* Christine: 
       * err_sqr = a^2 + b^2 + c^2 + ab + bc + ac
       * meaning: what's the e whose e^2 is the mean of err_a^2, err_b^2, and err_c^2
       * interpreted linearly in the triangle a, b, and c.
       */
      err_a = s_err[i][j];
      err_b = s_err[i][j+1];
      err_c = s_err[i+1][j];
      ...
      /* begin -- Christine Xu */
      abs_err_a = fabs(s_err[i][j]);
      abs_err_b = fabs(s_err[i][j+1]);
      abs_err_c = fabs(s_err[i+1][j]);      
      abs_err_tot += abs_err_a+abs_err_b+abs_err_c;      
      abs_err_sqr_tot += abs_err_a*(abs_err_a+abs_err_b+abs_err_c)+abs_err_b*(abs_err_b+abs_err_c)+abs_err_c*abs_err_c;
      /* end -- Christine Xu */
    }
  }
  ...
  for (i=1; i<n; i++) {
    for (j=1, jmax=n-i; j<jmax; j++) {
      ...
      err_sqr_tot += err_a*(err_a+err_b+err_c)+err_b*(err_b+err_c)+err_c*err_c;
      /* begin -- Christine Xu */
      abs_err_a = fabs(s_err[i-1][j]);
      abs_err_b = fabs(s_err[i][j-1]);
      abs_err_c = fabs(s_err[i][j]);      
      abs_err_tot += abs_err_a+abs_err_b+abs_err_c;      
      abs_err_sqr_tot += abs_err_a*(abs_err_a+abs_err_b+abs_err_c)+abs_err_b*(abs_err_b+abs_err_c)+abs_err_c*abs_err_c;
      /* end -- Christine Xu */
   }
  ...
  for (i=0; i<tse->n_samples_tot; i++) {
    ...
    /* begin -- Christine Xu */
    if (abs_err_min > fabs(err_local)) abs_err_min = fabs(err_local);
    if (abs_err_max < fabs(err_local)) abs_err_max = fabs(err_local);
    /* end -- Christine Xu */
  }
  ...
  /* begin -- Christine Xu */
  fe->abs_min_error = abs_err_min;
  fe->abs_max_error = abs_err_max;
  /* end -- Christine Xu */
  if (n != 1) { /* normal case, (n-1)*n/2+(n-2)*(n-1)/2 = (n-1)*(n-1) */
    /* begin -- Christine Xu */
    fe->abs_mean_error = abs_err_tot/((n-1)*(n-1)*3);
    fe->abs_mean_sqr_error = abs_err_sqr_tot/((n-1)*(n-1)*6);
    /* end -- Christine Xu */
    fe->mean_error = err_tot/((n-1)*(n-1)*3);    
    fe->mean_sqr_error = err_sqr_tot/((n-1)*(n-1)*6);
  } else { /* special case */
    /* begin -- Christine Xu */
    fe->abs_mean_error = fabs(tse->err_lin[0]);
    fe->abs_mean_sqr_error = tse->err_lin[0]*tse->err_lin[0]; // always non-negative
    /* end -- Christine Xu */
    fe->mean_error = tse->err_lin[0];
    fe->mean_sqr_error = tse->err_lin[0]*tse->err_lin[0]; // always non-negative
  }
  /* Update overall statistics */
  if (err_min < dss_stats->min_dist) dss_stats->min_dist = err_min;
  if (err_max > dss_stats->max_dist) dss_stats->max_dist = err_max;
  /* begin -- Christine Xu */
  if (abs_err_min < dss_stats->abs_min_dist) dss_stats->abs_min_dist = abs_err_min;
  if (abs_err_max > dss_stats->abs_max_dist) dss_stats->abs_max_dist = abs_err_max;
  /* end -- Christine Xu */
  
  dss_stats->mean_tot += fe->mean_error*fe->face_area;
  dss_stats->abs_mean_tot += fe->abs_mean_error*fe->face_area; 
  /* begin -- Christine Xu */
  dss_stats->rms_tot += fe->mean_sqr_error*fe->face_area;  
  dss_stats->abs_rms_tot += fe->abs_mean_sqr_error*fe->face_area;
  /* end -- Christine Xu */
...
/* Christine: When the sampling freq. passed in this function is 2, 
 * it means we just sample the 3 vertice of the triangle, 
 * and n*(n+1)/2 = 3 IMPORTANT!! */
          ...
          const dvertex_t *c, int n, struct sample_list* s)
{
  ...
  if (s->buf_sz < s->n_samples) {
    s->sample = (dvertex_t *)xa_realloc(s->sample,sizeof(*(s->sample))*s->n_samples);
                   ...
                   dvertex_t bbox_min)
{
  ...
  lst = (struct t_in_cell_list *)xa_malloc(sizeof(*lst));
  nt = (int *)xa_calloc(grid_sz.x*grid_sz.y*grid_sz.z,sizeof(*nt));
  tab = (int **)xa_calloc(grid_sz.x*grid_sz.y*grid_sz.z,sizeof(*tab));
  ecb = (int *)xa_calloc((grid_sz.x*grid_sz.y*grid_sz.z+EC_BITMAP_T_BITS-1)/
  ...
  for (i=0, imax=tl->n_triangles; i<imax;i++) {
    ...
    if (m_a == m_b && m_a == m_c && n_a == n_b && n_a == n_c &&
      ...
      tab[cell_idx] = (int *)xa_realloc(tab[cell_idx],(nt[cell_idx]+2)*sizeof(**tab));
    ...
    for(j=0;j<sl.n_samples;j++){
      ...
      if (cell_idx != cell_idx_prev) {
        if (c_buf_sz <= h) {
          c_buf = (int *)xa_realloc(c_buf, (h+1)*sizeof(*c_buf));
          c_buf_sz++;
        }
        c_buf[h++] = cell_idx;
    ...
    for (j=0; j<h; j++) {
      ...
      if (nt[cell_idx] == 0 || tab[cell_idx][nt[cell_idx]-1] != i) {
        tab[cell_idx] = (int *)xa_realloc(tab[cell_idx],
 ...
 * (grid_sz.x,grid_sz.y,grid_sz.z) cells in the X,Y,Z directions. Cell (0,0,0)
...
/* Christine: This is the function that does the space subdivision trick.
 */
                           ...
                           int **dcl_buf, int *dcl_buf_sz)
{
  ...
  do {
    ...
    for (cur_cell = dcl[cell_idx].list[k].cell,
      ...
      do { /* cell has always one triangle at least, so do loop is OK */
        ...
        stats->n_triag_scans++;
#endif
  //this is to calculate the distance between triags[t_idx] and point p
        dist_sqr = dist_sqr_pt_triag(&triags[t_idx],&p);
        if (dist_sqr < dmin_sqr) {
          //keep track of the t_idx and the closest point in triags to p
          track_idx = t_idx;
          dmin_sqr = dist_sqr;
        }
  ...
  //here to determine whether should return sqrt(dmin_sqrt) or -sqrt(dmin_sqrt)  
  substract_dv(&p,&(triags[track_idx].a),&ap);
  //make sure the normal of triags have been calculated, and from the code, it seems so.
  prod = scalprod_dv(&(triags[track_idx].normal),&ap);
  if(prod >= 0)
                    ...
                    struct prog_reporter *prog)
{
  ...
  //cell_sz: side length of the cubic cell
  cell_sz = get_cell_size(tl2,&bbox_min,&bbox_max,&grid_sz);
  dcl = (struct dist_cell_lists *)xa_calloc(grid_sz.x*grid_sz.y*grid_sz.z,sizeof(*dcl));
  dcl_buf = NULL;
  dcl_buf_sz = 0;
  ...
  me1->fe = (struct face_error *)xa_realloc(me1->fe,m1->num_faces*sizeof(*(me1->fe)));
  ...
  m_stats.dist_smpl =
    (double *)xa_malloc(sizeof(*(m_stats.dist_smpl))*m_stats.dist_smpl_sz);
#ifdef DO_DIST_PT_SURF_STATS
  ...
  for (k=0, kmax=m1->num_faces; k<kmax; k++) {
    ...
    if (me1->fe[k].face_area < DMARGIN*DBL_MIN) continue; /* degenerate */
  ...
  stats->mean_dist = stats->mean_tot/stats->st_m1_area;
  stats->rms_dist = sqrt(stats->rms_tot/stats->st_m1_area);
  /* begin -- Christine Xu */
  stats->abs_mean_dist = stats->abs_mean_tot/stats->st_m1_area;
  stats->abs_rms_dist = sqrt(stats->abs_rms_tot/stats->st_m1_area); 
  /* end -- Christine Xu */
  finalize_face_error(me1,&m_stats);
  me1->min_error = stats->min_dist;
  me1->max_error = stats->max_dist;
  me1->abs_min_error = stats->abs_min_dist;
  me1->abs_max_error = stats->abs_max_dist;
