/* $Id: model_in.cxx,v 1.2 2006/10/25 02:55:21 xushun Exp $ */
...
static int refill_buffer(struct file_data* data) 
{
  ...
  while (strchr(VRML_WS_CHARS, data->block[data->nbytes-1]) == NULL) {
    ...
    if (data->nbytes == data->size-1) { /* we need more space */
      data->block = (unsigned char *)grow_array(data->block, sizeof(unsigned char), 
...
int find_string2(struct file_data *data, const char *string)
{
  int c,i;
...
static int detect_file_format(struct file_data *data)
{
  ...
  if (c == '1'){ /* probably byu*/
    ...
    {
      c = ungetc(c,data);
      rcode = (c !=EOF) ? MESH_FF_BYU : MESH_CORRUPTED;
      return rcode;
    }
    else
    {
               ...
               int fformat, int concat, const char* fname)
{
  ...
  case MESH_FF_PLY:
    ...
    break;
  case MESH_FF_BYU:
    //rcode = read_byu_tmesh(&models, data);
    //rcode = read_byu_tmesh2(&models, data, fname);
    break;
  case MESH_FF_M3D:
  //rcode = read_m3d_tmesh(&models, data);
    break;
  default:
    //rcode = MESH_BAD_FF;
    break;
